...
def placement_of_groups_in_classrooms(groups_list,classrooms_list):
    ...
    for i in range(len(groups_list)):
        ...
        for j in range(len(classrooms_list)):
            add_males = classrooms_list[j][1] + groups_list[i][1]
            add_females = classrooms_list[j][2] + groups_list[i][2]
            add_specialneeds = classrooms_list[j][3] + groups_list[i][3]
            add_ggrades = classrooms_list[j][4] + groups_list[i][4]
            ...
            needs.append([j, coef1*add_males +coef2*add_females + coef3*add_specialneeds + coef4*add_ggrades ]) #* random.uniform(0.9, 1)])
...
for j in range(iterations):
    ...
    # I want to normalize the variances in order to add them. How can I do it? 
    solution_cost = variances[1]/males + variances[2]/females + variances[3]/special_needs + variances[4]/sum_of_grades
...
for key in sorted(output_dict.keys()):
    ...
    for index, row in df.iterrows():
        ...
        if str(row['GroupN']).strip() == 'Group_' + str(key):
            ...
            # print('Βάλε μαθητή ' ,index+1,' από το Gr
            # oup ',key, ' στην τάξη ', output_dict[key][0])
            pass
